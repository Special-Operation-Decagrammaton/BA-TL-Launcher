...
class SettingManager:
    ...
    def update_installed_patch_text(self):
        ...
        update_note = (manifest.PatchNote if manifest else "") or ""
        if "Generated by BA-TL-IMAGE" in update_note or "Cloudflare R2" in update_note:
            update_note = ""
        if not update_note:
            update_note = "No patch notes available." if not manifest else "-"
    ...
    def toggle_download_images(self, value: bool):
        self.app.game_config.DownloadImages = value
        save_config(self.app.game_config, CONFIG_PATH)